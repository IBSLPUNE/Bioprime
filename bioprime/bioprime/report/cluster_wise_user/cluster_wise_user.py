...
def get_conditions(filters, additional_conditions=None):
    ...
    conditions += f" and `tabSales Invoice`.territory = '{user_territory}'" if user_territory else ""
    user_territory = user_territory or 'default_territory'
...
def get_items(filters, additional_query_columns, additional_conditions=None):    
    ...
    return frappe.db.sql(
        ...
        where `tabSales Invoice`.name = `tabSales Invoice Item`.parent and
            ...
            and `tabSales Invoice Item`.item_code != 'Security Deposit'
            {1}
        """.format(
