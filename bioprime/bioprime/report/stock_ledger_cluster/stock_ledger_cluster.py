...
def get_user_territory(user):
    """Fetch the territory assigned to the user."""
    territory = frappe.db.get_value("custom_users", user, "custom_territory")
    if territory:
        return territory
    return None  # Return None if no territory is assigned

def execute(filters=None):
    # Proceed with fetching the stock ledger entries if the user has a territory (or if admin)
    is_reposting_item_valuation_in_progress()
...
def get_stock_ledger_entries(filters, items):
    ...
    # Fetch the user's assigned territory
    user_territory = get_user_territory(frappe.session.user)

    # Main query for Stock Ledger Entry
    query = (
        frappe.qb.from_(sle)
        .select(
            sle.item_code,
            ...
            sle.serial_no,
            sle.project,
            si.territory.as_("territory"),  # Fetch territory directly from the joined Sales Invoice
        )
        # Join Sales Invoice Item to Stock Ledger Entry via Delivery Note (voucher_no)
        .left_join(si_item)
        .on((si_item.delivery_note == sle.voucher_no) & (sle.voucher_type == "Delivery Note"))
        # Join Sales Invoice to Sales Invoice Item
        .left_join(si)
    ...
    if user_territory:
        query = query.where(si.territory == user_territory)
