...
def get_gl_entries(filters, accounting_dimensions):
	...
	gl_entries = frappe.db.sql(
		...
		select
			...
			gle.voucher_type, gle.voucher_no, {dimension_fields}
			gle.cost_center, gle.project,
			gle.against_voucher_type, gle.against_voucher, gle.account_currency,
			gle.remarks, gle.against, gle.is_opening, gle.creation {select_fields},
			CASE 
				WHEN party_type = 'Customer' THEN (SELECT customer_name FROM `tabCustomer` WHERE name = gle.party)
				WHEN gle.party_type = 'Supplier' THEN (SELECT supplier_name FROM `tabSupplier` WHERE name = gle.party LIMIT 1)
				WHEN gle.party_type = 'Employee' THEN (SELECT employee_name FROM `tabEmployee` WHERE name = gle.party LIMIT 1)
				ELSE gle.party 
			END as party_name
